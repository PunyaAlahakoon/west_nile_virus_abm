...
sample_pools_and_calc_cts<-function(all_data,gridd){
  ...
  avg_sizes<-read.csv("sample_sizes_by_surevil_calc.csv")
  ...
  calc_pooled_ct<-function(decay_rate,time,week,year,
    ...
    colnames(cts_per_pool)=c("decay_rate","time","week", "year","sample_size", "pooled_ct_per_pool")
    for(j in 1:(n_pools)){
      ...
      ct_ss=min(ct_x,40) 
      cts_per_pool<-rbind(cts_per_pool,data.frame("decay_rate"=decay_rate,"time"=time ,"week"=week, "year"=year,
  ...
  colnames(pool_numbers)<-c("decay_rate","time","week", "year","number_of_pools")
  
 # griddx=subset(gridd,year==yr_to_run & decay_rates==decay_rate_to_run )
  griddx=gridd
  ...
  for (i in 1:nrow(griddx)) { #just run one sim for the moment, if not, use 1:nrow(gridd)
    ...
    mozzies_at_i<-subset(blood_seeking_mozzies,blood_seeking_mozzies$current_time==griddx$days[i] & blood_seeking_mozzies$decay_rate==griddx$decay_rates[i] & blood_seeking_mozzies$year==griddx$year[i] )
    ...
    if(n_pools>0 ){
      ...
      cts_pools<-calc_pooled_ct(decay_rate=griddx$decay_rates[i],time=griddx$days[i],week=griddx$week[i],year=griddx$year[i],
                                n_pools=n_pools,r_mozzies,sample_size_at_i,mozzies_at_i)
      cts_per_pool<-rbind(cts_per_pool,cts_pools)
    }
    pool_numbers<-rbind(pool_numbers,data.frame("decay_rate"=griddx$decay_rates[i],"time"=griddx$days[i] ,
                                                "week"=griddx$week[i], "year"=griddx$year[i],"number_of_pools"=n_pools))
  }
