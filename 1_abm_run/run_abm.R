...
n_sims<-4#number of sims to run 
...
vira_per_ch<-c(1,.1,.01,.001,0) # probability of the amount of virus transferred to the mozzie from the bird 
grds<-expand.grid(change_prob_model,vira_per_ch)
...
for (i in 1:n_sims) { #if running PART X:, chnage n_sims to nrow(grds)
  ...
  prob_ct_model_change<-grds[4,1]#if the mozzie_viral_dynamics_method==2
  viral_percent_from_birds<-grds[4,2]
  
  #run the sim several times:
  run_mozzie_model( time,  nHosts,  nMozzies,  daily_pr_bite,mozzie_birth_rate_t,
                    probHostInfection,  probMozzInfection_t,  iniInfectedMozzies,
                    host_birth_rate,
                    host_death_rate,vertical_transms_prob,
                    daily_pr_death,seed_on,seed_mozzies,
                    i,egg_to_adult_days,
                    mozzie_viral_dynamics_method, prob_ct_model_change,
                    viral_percent_from_birds,set_the_overwintering_period,
                    mozzie_owerwintering_prob,
                    set_the_mozzie_active_period,mozzie_becoming_active_prob,
                    FALSE)
